...
function playLoop() {
  ...
  function firstColumn() {
    ...
    for (let i = 0; i < rows; i++) {
      console.log(
        `row ${i}, Column ${currentColumn} → ${gridArray[i][currentColumn]}`,
      );
      let buttonIndex = i * columns + currentColumn; // Find buttonIndex from 2D array
      buttons[buttonIndex].classList.add("highlighted");
      // Play Kick
      if (
        buttons[buttonIndex].dataset.type === "kick" &&
        buttons[buttonIndex].value === "active"
      ) {
        console.log("play kick");
        playKick();
      } else if (
        buttons[buttonIndex].dataset.type === "closed_hat" &&
        buttons[buttonIndex].value === "active"
      ) {
        console.log("play kick");
        playClosedHat();
      } else if (
        buttons[buttonIndex].dataset.type === "clap" &&
        buttons[buttonIndex].value === "active"
      ) {
        console.log("play kick");
        playClap();
...
function playKick() {
  ...
  gainNodeKick.gain.setValueAtTime(0.1, kick.currentTime);
...
function playClosedHat() {
  ...
  gainNodeClosedHat.gain.linearRampToValueAtTime(
    0.01,
    closedHat.currentTime + 0.05,
  );
...
function playClap() {
  ...
  gainNodeClap.gain.setValueAtTime(0.1, clap.currentTime);
